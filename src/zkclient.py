...
import zookeeper, time, threading, conf
...
class ZKClient(object):
    def __init__(self, servers, timeout=DEFAULT_TIMEOUT):
        ...
        conf.logger.debug("Connecting to %s" % (servers))
        start = time.time()
        ...
        conf.logger.debug("Connected in %d ms, handle is %d"
    ...
    def create(self, path, data="", flags=0, acl=[ZOO_OPEN_ACL_UNSAFE]):
        ...
        conf.logger.debug("Node %s created in %d ms"
    ...
    def delete(self, path, version=-1):
        ...
        conf.logger.debug("Node %s deleted in %d ms"
                          % (path, int((time.time() - start) * 1000)))
        return result
...
class CountingWatcher(object):
    ...
    def __call__(self, handle, typ, state, path):
        self.count += 1
        conf.logger.debug("handle %d got watch for %s in watcher %d, count %d" %
